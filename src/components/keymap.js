    ...
    return (
      <div>
        <KeySection title="Modify">
          ...
          <KeyInfo keys={['Ctrl + d']}>Delete {itemType}</KeyInfo>
          <KeyInfo keys={['Alt + ⬆']}>Move {itemType} up</KeyInfo>
          <KeyInfo keys={['Alt + ⬇']}>Move {itemType} down</KeyInfo>
              ...
              ([checkFn, keys, label]) =>
                !!checkFn(selectedNode) && (
                  <KeyInfo
                    key={label}
                    keys={
                      selectedIsNullLiteral || selectedIsBooleanLiteral
                        ? keys
                        : ['Alt + ' + keys[0]]
                    }
                  >
